...
struct Light {
    ...
    void setupCameraForShadowMapping(glm::vec3 scene_center , float scene_radius) {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(90.0),(double)window_width/(double)window_height,1.0,10.0);

        this->depthMVP = projection * view;
    }
...
struct Scene{
    ...
    void render() {
        ...
        for(unsigned int i = 0; i < NB_LIGHTS; i++)
        {
            ...
            shaderProgramPtr->set(std::string("lightSources[" + std::to_string(i) + "].depthMVP"),light.depthMVP);

            // Set texture for the shadowmap
...
void initScene (const std::string & meshFilename) {
    ...
    // Setup lights :
    {
        ...
        for(unsigned int i = 0; i < NB_LIGHTS; i++,offset++) {
            ...
            if(i % 2 == 0) {
                ...
                newLight.m_position = glm::vec3 (-2*cos(app_timer) + offset, 2*sin(app_timer) + offset, 3.5);
...
void update (float currentTime) {
    if( ! app_timer_is_stopped ) {
        ...
        for(unsigned int i = 0; i < NB_LIGHTS; i++,offset++) {
            ...
            if(i % 2 == 0) { // Rotation sens horaire

                scene.scene_lights[i].m_position = glm::vec3 (-2*cos(app_timer) + offset, 2*sin(app_timer) + offset, 3.5);
            
            } else { // Rotation sens anti-horaire
