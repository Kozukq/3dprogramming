...
public:
    ...
    static float euclideanDistance(const Vec3& a, const Vec3& b) {
        return sqrt((a.mVals[0] - b.mVals[0]) * (a.mVals[0] - b.mVals[0]) + (a.mVals[1] - b.mVals[1]) * (a.mVals[1] - b.mVals[1]) + (a.mVals[2] - b.mVals[2]) * (a.mVals[2] - b.mVals[2]));
    }
    void operator += (Vec3 const & other) {
        mVals[0] += other[0];
        mVals[1] += other[1];
