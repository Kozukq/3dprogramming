...
void updateSystem() {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
            ...
            // Y
            arapLinearSystem.A(equationIndex,3 * v + 1) = -1;
            arapLinearSystem.A(equationIndex++,3 * vNeighbor + 1) = 1;

            // Z
            arapLinearSystem.A(equationIndex,3 * v + 2) = -1;
            arapLinearSystem.A(equationIndex++,3 * vNeighbor + 2) = 1;
        }
    }
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        if(verticesHandles[v] != -1) {
...
void mouse (int button, int state, int x, int y) {
    if( glutGetModifiers() & GLUT_ACTIVE_SHIFT    ||   rectangleSelectionTool.isActive ) { // we can activate the selection only with ctrl pressed
        if( viewerState == ViewerState_EDITINGHANDLE ) {
            if (state == GLUT_UP) {
