...
void setUnitSphere(Mesh& o_mesh, int nX, int nY) {
    ...
    // Create vertices and normals at each vertex

    for(int posY = 0; posY <= nY; ++posY) {

        for(int posX = 0; posX <= nX; ++posX) {

            float theta = posX * 2 * M_PI / nX;
            float phi = - M_PI / 2 + posY * M_PI / nY;
    ...
    for(int i = 0; i < vertices_amount - nX - 1; ++i) {

        unsigned int vertex1 = i;
        unsigned int vertex2 = i + 1;
        unsigned int vertex3 = i + nX + 1;

        unsigned int vertex4 = i;
        unsigned int vertex5 = i + nX + 1;
        unsigned int vertex6 = i + nX;

        o_mesh.triangles.push_back(Triangle(vertex1,vertex2,vertex3));
        o_mesh.triangles.push_back(Triangle(vertex4,vertex5,vertex6));
